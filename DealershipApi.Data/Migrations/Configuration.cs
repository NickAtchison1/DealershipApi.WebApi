    ...
    internal sealed class Configuration : DbMigrationsConfiguration<DealershipApi.Data.DataModels.ApplicationDbContext>
    {
        ...
        protected override void Seed(DealershipApi.Data.DataModels.ApplicationDbContext context)
        {
            ...
            var vehicles = new List<Vehicle>
            {
                ...
                new Vehicle { Make ="Nissan", Model="Maixma", ModelYear=2019, Color="White", InvoicePrice=9800, DealershipId = dealerships.Single(d => d.Name == "Shay's Car Emporium Chevy North").Id, InStock = true, VehicleCondition=VehicleCondition.Used, Mileage=15000 },
            ...
            var rand = new Random();
            int range = 365;
            // var roles = context.Roles.Single(r => UserLoginInfo.)
            // DateTime randomDate = DateTime.Today.AddDays(-rand.Next(range));
            foreach (var vehicle in vehicles)
            {
                vehicle.CreatedDate = DateTime.Today.AddDays(-rand.Next(range));
                vehicle.UpdatedDate = vehicle.CreatedDate;
                vehicle.CreatedBy = (from u in context.Users
                                         //from ur in u.Roles
                                         //join r in context.Roles on ur.RoleId equals r.Id
                                         //where r.Name == "Admin"
                                     where u.DealershipId == vehicle.DealershipId
                                     where new[] { "Nick", "Landon", "Louis", "Andrew" }.Any(s => s == u.FirstName)
                                     select u.Id).Single();
                vehicle.UpdatedBy = vehicle.CreatedBy;

                //  vehicle.CreatedBy = context.Users.Single(u => u.DealershipId.ToString() == vehicle.DealershipId.ToString()).ToString();
